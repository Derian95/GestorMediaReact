...
export const ModalMedia = () => {
	...
	const { idImagenVideo, title, description, location, defaultMedia } =
		useMedia()
	...
	const postMediaData = async (infoMedia: any) => {
		...
		const response = await axios.post(
			'http://localhost:3005/apiGestor/Sgc/addMedia',
			infoMedia
		)
	...
	const editMediaData = async (infoMedia: any) => {
		...
		const response = await axios.put(
			'http://localhost:3005/apiGestor/Sgc/updateMedia',
			infoMedia
		)
		const result = response.data
		closeModal()
	...
	return (
		<ModalContainer>
			<div
				...
				} max-h-screen overflow-y-scroll  w-5/6 mt-9 md:w-2/4 xl:w-4/12 bg-white shadow-2xl p-6`}>
				<div className='flex  justify-between mb-8'>
					<h1 className='text-2xl'>AGREGAR PUBLICIDAD</h1>
					<div onClick={changeImage} className='cursor-pointer text-3xl'>
						x
					</div>
				</div>
				<form onSubmit={handleSubmit(onSubmit)}>
					<div className='w-full flex flex-col mb-4'>
						...
						{formState.errors.titulo && (
							<span className='text-error text-sm'>Título es obligatorio</span>
						)}
					</div>
					...
					<div className='w-full flex flex-col mb-4'>
						...
						<textarea
							className='bg-white rounded-lg p-2 border-[1px] border-[#8c8b8b]'
							id='descripcion'
					...
					<div className='w-full flex flex-col '>
						...
						/>
						{formState.errors.imagen && (
							<span className='text-error text-sm'>Imagen es obligatoria</span>
						)}
					</div>
					...
					<button
						className={`${
							loader ? 'bg-gray' : 'bg-base hover:bg-baseH'
						}  text-white  rounded w-full flex gap-2 items-center justify-center`}
						type='submit'
						disabled={loader}>
						{loader ? (
							<Lottie
